...
class Yolo:
    ...
    def filter_boxes(self, boxes, box_confidences, box_class_probs):
        """
        Public method that filter boxes
        Args:
            boxes: ndarrays of shape (grid_height, grid_width,
            ...
            box_class_probs: list of numpy.ndarrays of shape (grid_height,
                             ...
                             output, respectively
        Returns: Tuple of (filtered_boxes, box_classes, b_scores)
        """
        b_score = []

        for b_conf, b_probs in zip(box_confidences, box_class_probs):
            score = (b_conf * b_probs)
            b_score.append(score)
        # Finding the index of the class with maximum box score
        box_classes = [s.argmax(axis=-1) for s in b_score]
        box_class_l = [b.reshape(-1) for b in box_classes]
        box_classes = np.concatenate(box_class_l)

        # Getting the corresponding box score
        box_class_scores = [s.max(axis=-1) for s in b_score]
